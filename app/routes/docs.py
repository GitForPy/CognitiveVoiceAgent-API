...
from app.schema import ChatSchema, InputSchema, ErrorSchema

def create_tags(spec):
    """Создаем теги для Swagger."""
    tags = [{'name': 'main', 'description': 'Основные запросы'}]
    for tag in tags:
        spec.tag(tag)

def load_docstrings(spec, app):
    """Загружаем описание API из docstrings."""
...
def get_apispec(app):
    ...
    spec.components.schema("InputSchema", schema=InputSchema)
